from ophyd_async.core import AsyncStageable, AsyncStatus
...
class KickerPS(RawKickerPS, _SettableControllingDifference, AsyncStageable, Stoppable):
    def __init__(self, *args, deactivate_on_stop: bool = True, **kwargs):
        ...
        _SettableControllingDifference.__init__(
            self, *args, eps_abs=eps_abs, eps_rel=eps_rel, **kwargs
        )
    ...
    async def stage(self):
        ...
        assert (
            hv == HighVoltageState.ON
        ), f"{self.__class__.__name__} high voltage is not on"
    ...
    async def set(self, new_position: float, timeout: float = 30.0):
        ...
        # Todo: .set is an AsyncStatus should one wait on .task
        return await _SettableControllingDifference.set(
            self, new_position, timeout=timeout
        )
